...
We investigate whether emotion-specific residuals $\hat{\mathbf{r}}_e$ causally bias text generation towards the target emotion, and whether incorporating the shared vector $\mathbf{g}$ at any scale yields further benefits. We generate follow-up sentences using \texttt{Llama-3.1-8B-Instruct} from 100 source utterances sampled from the test split of the emotion rewriting dataset. To eliminate sampling variance as a confounding factor, we employ greedy decoding (temperature $= 0$) throughout the entire process.
...
\begin{enumerate}
  ...
  With $\alpha_G = 0$ fixed (to isolate the effect of the residual component), we scan $\alpha_R$ over seventeen evenly-spaced values, $\{-128,\,-112,\,-96,\,-80,\,-64,\,-48,\,-32,\\\,-16,\,0,\,16,\,32,\,48,\,64,\,80,\,96,\,112,\,128\}$. The anchor $\alpha_R = 0$ replicates the unsteered baseline. Negative values steer \textbf{away} from the target emotion, and large positive values are expected to approach or exceed the coherence threshold at which generated text degrades. Each combination of source text, target emotion, and $\alpha_R$ value constitutes a single generation, giving $100 \times 8 \times 17 = 13{,}600$ generated texts.
  \item \textbf{$\alpha_G$ sweep} \\
  Based on the exploration on $\alpha_R$ value before, we fix $\alpha_R = 64.0$ as the residual scale, and scan $\alpha_G \in \{0, 2.0, 4.0, 8.0\}$.  The anchor condition $\alpha_G = 0$ corresponds to pure residual steering with no shared component.  The intermediate values ($2.0$ and $4.0$) test whether contributions from $\mathbf{g}$ below a certain threshold are beneficial before the scaling becomes detrimental. The same 100 test-split sources and 8 target emotions are used, giving $100 \times 8 \times 4 = 3{,}200$ generated texts.
\end{enumerate}

\subsection{Automated Evaluation Protocol}

Each generated sentence is scored by \texttt{gpt-4o-mini}, which acts as a judge. To ensure consistent evaluation criteria across all items, the judge is presented with three fixed few-shot examples that cover the entire scoring range. Specifically, these are:

\begin{itemize}
  \item An example that match a high-quality target
...
These examples serve to anchor the two endpoints of the evaluation scale before the evaluator views the test items. Scores are retrieved via OpenAI's Structured Output API, which utilises a strict JSON schema. This eliminates the need for free-form parsing and ensures that the three numerical scores and the dominant sentiment label are always returned in a consistent, machine-readable format. The evaluators operate without being informed of the intervention condition ($\alpha_G$ or $\alpha_R$).\par

For the $\alpha_R$ sweep, the below three continuous $[0,1]$ metrics are collected per item:
\begin{itemize}
  \item \textbf{Sentence soundness}: Whether the generated rewrite is coherent, grammatically correct and free of repetition. This serves as a quality check to flag outputs that are incoherent or degenerate, which could otherwise confound the interpretation of the other metrics.
...
The actual system prompt used for the evaluation is shown in Appendix1. 
...
Figure~\ref{fig:alpha_r_sweep_aggregate} shows all three metrics pooled across the eight emotions. Soundness remains at or above $0.70$ for $\alpha_R \in [-96,\, 80]$; outside this interval, generated text degrades into repetition or incoherence. Within the coherent window, target-emotion intensity rises monotonically from the unsteered baseline ($0.339 \pm 0.020$ at $\alpha_R = 0$) and peaks at $\alpha_R = 80$ ($0.581 \pm 0.019$), before declining at $\alpha_R = 96$ as coherence fails. Semantic preservation tracks soundness closely, falling from $0.912$ at $\alpha_R = 0$ to $0.677$ at $\alpha_R = 80$, reflecting the cost of larger perturbations on content fidelity.\par

In addition to the coherent range itself, we report two summary metrics for each emotion. The arithmetic midpoint $M_A=(L_e+R_e)/2$ measures whether the consistent window is symmetrical about the unsteered baseline. A value close to zero indicates that the headroom for positive and negative steering is similar, whilst a deviation from zero suggests an asymmetric steering range. We also report on $M_B$, namely the value of $\alpha_R$ that maximises semantic preservation within the coherent range. This identifies the point at which the generated text maintains the closest alignment with the source text's meaning.\par
...
As shown in Table~\ref{tab:alpha_r_thresholds} and Figure~\ref{fig:alpha_r_sweep_thresholds}, the range of $\alpha_R$ within which text remains readable and the optimal operating point vary significantly depending on the emotion. Sadness has the widest coherent range (the entire sweep, $[-128, 128]$), whilst trust and surprise are the most constrained (width 160).\par

There are two emotions that exhibit particularly asymmetric profiles: anger and disgust. For both of these, the midpoint of the coherent range is slightly negative ($M_A=-16$ and $M_A=-24$), whereas the point with the highest semantic conservation is positive ($M_B=48$). Consequently, $\alpha_R=0$ is not the natural centre of these steering axes. Instead, as shifting slightly in the positive direction better preserves the original meaning, this suggests that these high-arousal negative residuals do not lie precisely at the centre of the unsteered distribution.\par

At $\alpha_R = 64$, which falls within a coherent range for all emotions, raters correctly identified the dominant emotion in 90\% of anger cases and 86\% of joy cases. The figures for fear and anticipation were 78\% and 66\% respectively. Disgust (45\%), sadness (46\%) and, in particular, surprise (16\%) were frequently classified as neutral even on this scale, indicating that these residual directions are causing stylistic shifts whereby they are not perceived by evaluators as strong emotions.\par
...
Trust exhibits a distinctive response pattern. Unlike all other emotions, the intensity of this emotion peaks at $\alpha_R = 16$ ($0.546$) and then decreases monotonically as the positive value increases, reaching $0.181$ at $\alpha_R = 96$. Furthermore, the lowest point of emotional intensity within a coherent range occurs at the positive value $\alpha_R = +96$. This implies that inducing ``towards trust'' paradoxically suppresses the expression of trust. \par
One possible explanation is that trust overlaps with the default assistant persona induced by instruction tuning and RLHF. Such models are trained to follow user intent and produce helpful, preference-aligned responses \citep{bai2022traininghelpfulharmlessassistant,ouyang2022traininglanguagemodelsfollow}, so trust-like language (e.g. reassurance, deference, and cooperation) may already be strong in the unsteered baseline. This is consistent with the high baseline trust intensity of $0.536$ at $\alpha_R = 0$.\par
Increasing $\alpha_R$ may therefore overshoot this default register, pushing the model toward excessive reassurance or sycophantic agreement rather than stronger perceived trust. Prior work shows that human-feedback-trained models can learn to affirm user views in ways that are preferred but not necessarily truthful or appropriate \citep{wei2024simplesyntheticdatareduces,sharma2025understandingsycophancylanguagemodels}. Thus, the decline in trust intensity at larger positive $\alpha_R$ may reflect a shift from interpretable trust into unnatural flattery.\par

\subsubsection*{Negative $\alpha_R$ and Bipolar Steering}
Steering at $\alpha_R < 0$ suppresses the target emotion whilst maintaining coherence within $[L_e, 0]$. The four discrete high-arousal emotions (anger, fear, disgust and surprise) can be reduced to near zero ($\mathrm{TI} \approx 0.000$–$0.006$) whilst maintaining validity at $0.70$ or higher at their most negative coherent $\alpha_R$. The suppression of positive and social emotions is more difficult: joy maintains $\mathrm{TI} = 0.130$ at $\alpha_R = -80$, and anticipation maintains $0.323$ at $\alpha_R = -80$. These findings support the notion that $\hat{\mathbf{r}}_e$ encodes a bipolar emotional axis, but the difficulty of suppressing these emotions is asymmetrical across valence categories.
...
Figure~\ref{fig:alpha_g_sweep_aggregate} shows all three metrics as a function of $\alpha_G$. All three decline monotonically. Target-emotion match falls from $0.471 \pm 0.023$ at $\alpha_G = 0$ to $0.396 \pm 0.023$ at $\alpha_G = 2$, $0.314 \pm 0.023$ at $\alpha_G = 4$, and effectively zero at $\alpha_G = 8$, where the model produces incoherent or empty output. Semantic preservation declines from $0.794$ to $0.670$ over $\alpha_G \in [0, 4]$, and emotionality follows the same pattern ($0.625$ to $0.477$). Paired $t$-tests confirm that the degradation is statistically reliable at every tested scale: the drop at $\alpha_G = 2$ alone is $\Delta = -0.075$ ($t = -7.68$, $p = 1.55 \times 10^{-14}$, $n = 800$ pairs). The sole exception is joy, which shows a marginal increase at $\alpha_G = 2$ ($+0.020$), but this falls within the 95\% confidence interval ($\pm 0.023$) and is not statistically significant. No value of $\alpha_G > 0$ produces a reliable improvement on any metric, and the per-emotion breakdown (Figure~\ref{fig:alpha_g_sweep_per_emotion}) confirms that no individual emotion benefits from a positive $\alpha_G$ at any tested scale.

The shared direction $\mathbf{g}$ is useful as an analytical component for isolating emotion-specidic residuals. However, it is counterproductive as an additive generation-time steering component at every tested scale. Effective affective steering therefore reduces to $\Delta_e = \alpha_R\,\hat{\mathbf{r}}_e$, with $\alpha_G$ discarded.

\subsection{Qualitative Results} 
\label{sec:steering_causal_validation} 

Table~\ref{tab:cross_sadness_anger} presents representative qualitative examples of how basic sentences expressing sadness were steered towards the direction of anger. This example demonstrates three patterns that align with the quantitative sweep results. Firstly, increasing $\alpha_R$ alters not only the emotional vocabulary but also the evaluative framework of the sentences. At $\alpha_R=16$, the output remains close to grief and regret. The photographs become \textit{unforgettable memories} of irretrievable memories. At $\alpha_R=64$, the same loss is reframed as an injustice. Through the expression \textit{what was taken from me}, the event comes to be felt as a grievance or dissatisfaction rather than a passive lack. Thus, the anger residual not only adds angry words but also alters the causal interpretation of the loss.\par

Secondly, negative steering directs the output in the opposite emotional direction. With $\alpha_R=-64$, the sentence no longer emphasises injustice or dissatisfaction. Instead, through expressions such as \textit{bittersweet nostalgia} and \textit{what I have cherished}, the sense of loss is softened into a nostalgic acceptance. This supports the interpretation that $\hat{\mathbf{r}}_e$ functions as a bipolar emotional axis. That is to say, positive values intensify the target emotion, whilst negative values suppress or negate it.

Thirdly, excessive intervention leads to a decline in the quality of the generated text. At $\alpha_R=128$, the output becomes repetitive and semantically unstable, which is consistent with the decline in validity observed in the sweep experiments. A similar deterioration is observed when the shared component $\mathbf{g}$ is added: at $\alpha_G=4$, sentences become incomplete, and at $\alpha_G=8$, they fall into repetition. This qualitative pattern supports the quantitative finding that $\mathbf{g}$ is counterproductive as a control component during generation. However, $\mathbf{g}$ is useful for the analytical purpose of separating shared emotionality from emotion-specific residuals.
...
Two main conclusions can be drawn from the results of this sweep experiment. Firstly, whilst the shared direction $\mathbf{g}$ is useful for analysis, it is not suitable for use during generation. Although $\mathbf{g}$ captures a common shift from neutrality towards emotion, adding it during generation consistently reduces the degree of alignment with the target emotion, the preservation of meaning, and the emotionality of the output. Consequently, effective control is reduced solely to the residual term:
\[
\Delta_e = \alpha_R \hat{\mathbf{r}}_e .
\]

Secondly, the residual direction acts as an emotion-dependent affective axis that is controllable. Increasing $\alpha_R$ enhances the target emotion to the limit of consistency, whilst a negative $\alpha_R$ suppresses it, supporting a bipolar interpretation of $\hat{\mathbf{r}}_e$. In practice, $\alpha_R=64$ provides a conservative, high-coherence setting, whilst $\alpha_R=80$ yields stronger emotional intensity, albeit at a greater semantic cost. The exceptions are telling: trust peaks at a much smaller value ($\alpha_R=16$), suggesting an overlap with the default assistant register induced by instruction alignment and RLHF. Surprise, on the other hand, remains difficult to amplify even at the optimal setting.\par

\section{Limitations and Transition}

These results confirm that emotional steering can be implemented as an intervention in the decomposed residual space, and that the valid operating points are $\alpha_G = 0$ and $\alpha_R > 0$. The shared direction $\mathbf{g}$ is counterproductive across all metrics examined, and only the emotion-specific residual $\hat{\mathbf{r}}_e$ provides a reliable steering signal. This conclusion was derived precisely because the decomposition allowed the two components to be separated and verified independently. \par

 However, these results also reveal two limitations. Firstly, the steering effect is not absolute but modest. Although the degree of alignment with the target emotion improves consistently, it falls far short of an upper limit, and the intervention effect is constrained by the semantic and emotional content of the source utterance. Secondly, the magnitude of the effect varies significantly depending on the emotion, suggesting that the discriminative power of $\hat{\mathbf{r}}_e$ differs across emotion categories. This is a structural issue addressed in the next chapter. \par

 These limitations provide the motivation for the next chapter. Until now, $\hat{\mathbf{r}}_e$ has been treated as a single entity. In the next chapter, we will examine what this residual actually represents. In particular, we will consider whether emotions such as joy, sadness and anger should be treated as atomic vectors, or whether they contain local sub-axes corresponding to more subtle, pre-linguistic affective vectors.\par