...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{figures/alpha_r_sweep_fig2_per_emotion_intensity.png}
\caption{Per-emotion target-emotion intensity vs.\ $\alpha_R$ ($\alpha_G = 0$ fixed; $n = 100$ per cell; error bars = 95\% CI). Anger and joy show the steepest gains; surprise has the lowest peak intensity across the sweep.}\label{fig:alpha_r_sweep_per_emotion}
\end{figure}
...
Table~\ref{tab:cross_sadness_anger} illustrates the behavioural effects of varying $\alpha_R$ and $\alpha_G$. As $\alpha_R$ increases from $0$ to $64$, the output shifts from a passive expression of loss to an active expression of grievance. The phrase ``the injustice of never being able to turn back time and reclaim what's been taken from me'' reframes the loss as something done to the speaker, shifting the affective stance from sorrow to outrage. At $\alpha_R = -64$, the loss is softened into nostalgic acceptance: ``a bittersweet sense of nostalgia, but also a gentle reminder of the things I've cherished.''\par
...
\bottomrule
\end{tabular}
\caption{Sadness seed steered with $\hat{\mathbf{r}}_\text{anger}$.}
