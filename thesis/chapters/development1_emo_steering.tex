...
%
% TODO [prerequisite]: Run the following to produce the data for this section:
%   python exp14_alpha_r_sweep.py --merge <batch_raw.jsonl>
%   python exp14_alpha_r_sweep.py --analyse    → alpha_r_sweep_summary.csv, alpha_r_thresholds.csv
%   python exp14_alpha_r_sweep.py --plot       → fig1_aggregate_metrics.png,
%                                                fig2_per_emotion_intensity.png,
%                                                fig3_per_emotion_thresholds.png

% TODO [fig:alpha_r_sweep_aggregate]: Add figure: fig1_aggregate_metrics.png.
%   \begin{figure}[H]
%   \centering
%   \includegraphics[width=\linewidth]{figures/alpha_r_sweep_fig1_aggregate_metrics.png}
%   \caption{Aggregate soundness, meaning preservation, and target-emotion intensity
%            as a function of $\alpha_R$ ($\alpha_G = 0$ fixed; $n = 800$ per condition;
%            error bars = 95\% CI).
%            [Describe shape: does intensity peak at intermediate α_R then fall?
%             Does soundness drop sharply at the coherence thresholds?
%             Note the α_R=0 unsteered baseline for each metric.]}
%   \label{fig:alpha_r_sweep_aggregate}
%   \end{figure}

% TODO [aggregate prose]: Report the pooled coherent range.  Template:
%   "Soundness remains above $0.70$ in the range $\alpha_R \in [L_{\text{pool}},\, R_{\text{pool}}]$;
%    outside this range generated text becomes repetitive or incoherent.
%    Within the coherent range, target-emotion intensity peaks at
%    $\alpha_R = \langle\text{value}\rangle$ ($\langle\text{intensity}\rangle \pm \langle\text{CI}\rangle$),
%    then declines as coherence degrades."

% TODO [fig:alpha_r_sweep_per_emotion]: Add figure: fig2_per_emotion_intensity.png.
%   \begin{figure}[H]
%   \centering
%   \includegraphics[width=\linewidth]{figures/alpha_r_sweep_fig2_per_emotion_intensity.png}
%   \caption{Per-emotion target-emotion intensity vs.\ $\alpha_R$
%            ($\alpha_G = 0$ fixed; error bars = 95\% CI).
%            [Note which emotions peak earliest/latest and flag non-monotonic curves.]}
%   \label{fig:alpha_r_sweep_per_emotion}
%   \end{figure}

% TODO [fig:alpha_r_sweep_thresholds]: Add figure: fig3_per_emotion_thresholds.png.
%   \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.9\linewidth]{figures/alpha_r_sweep_fig3_per_emotion_thresholds.png}
%   \caption{Per-emotion coherence thresholds.  Each subplot shows soundness (green, left axis)
%            and target-emotion intensity (blue, right axis) vs.\ $\alpha_R$.
%            Vertical markers: $L_e$ / $R_e$ (outermost $\alpha_R$ with soundness $\geq 0.70$);
%            $M_A = (L_e + R_e)/2$ (arithmetic midpoint);
%            $M_B$ ($\alpha_R$ maximising meaning preservation within the coherent range).}
%   \label{fig:alpha_r_sweep_thresholds}
%   \end{figure}

% TODO [tab:alpha_r_thresholds]: Fill one row per emotion from alpha_r_thresholds.csv.
%   \begin{table}[H]
%   \centering
%   \small
%   \begin{tabular}{lrrrrrr}
%   \toprule
%   \textbf{Emotion} & $L_e$ & $R_e$ & $M_A$ & $M_B$ &
%     $\alpha_R^{+}$ (opt) & $\alpha_R^{-}$ (opt) \\
%   \midrule
%   % joy / trust / fear / surprise / sadness / disgust / anger / anticipation
%   \bottomrule
%   \end{tabular}
%   \caption{Per-emotion coherence thresholds at layer 13 ($\alpha_G = 0$,
%            soundness threshold $= 0.70$).
%            $L_e$ / $R_e$: leftmost/rightmost coherent $\alpha_R$.
%            $M_A$: arithmetic midpoint of coherent range.
%            $M_B$: $\alpha_R$ of peak meaning preservation (closest to unsteered content).
%            $\alpha_R^{\pm}$: $\alpha_R$ maximising target-emotion intensity on each side.}
%   \label{tab:alpha_r_thresholds}
%   \end{table}

% TODO [threshold prose]: Discuss where $\alpha_R = 5$ (used in the $\alpha_G$ sweep) falls.
%   Is it conservative relative to $M_A$, near the optimum, or past the intensity peak?
%   Does $M_A \approx 0$ (window symmetric) or is it shifted positive/negative?
%   What does an asymmetric window imply about the geometry of $\hat{\mathbf{r}}_e$?

% TODO [negative α_R prose]: Report whether coherent counter-steered rewrites emerge.
%   Fill from the dominant-emotion distribution at the most negative coherent $\alpha_R$.
%   Template: "At $\alpha_R = \langle\text{most negative coherent value}\rangle$,
%   soundness remains $\langle\text{value}\rangle$ but the dominant emotion shifts
%   from the target toward \langle counter-emotion\rangle, suggesting that
%   $\hat{\mathbf{r}}_e$ encodes a bipolar affective axis."

% TODO [t-test prose]: Report paired $t$-test (vs $\alpha_R = 0$) for the optimal
%   positive and best negative coherent $\alpha_R$.  Template:
%   "A paired $t$-test ($n = \langle\rangle$) confirms that
%    $\alpha_R = \langle\text{opt}\rangle$ reliably exceeds the unsteered baseline
%    on target-emotion intensity ($\Delta = \langle\rangle$, $t = \langle\rangle$,
...
Table~\ref{tab:qualitative} illustrates the behavioural effects of varying $\alpha_R$ and $\alpha_G$. 
...
% TODO [α_R sweep interpretation — insert paragraphs after the block above]:
%
%   (1) Operating-point location: Report where α_R = 5 sits relative to the coherent range.
%       Template: "The $\alpha_R$ sweep identifies $\alpha_R = \langle\text{opt}\rangle$ as the
%       near-optimal operating point; $\alpha_R = 5$ (used in the $\alpha_G$ sweep) lies
%       <conservative / near-optimal / past peak> relative to both the coherence midpoint
%       $M_A = \langle\rangle$ and the intensity peak."
%
%   (2) Window symmetry: Comment on whether $M_A \approx 0$ or is shifted.
%       A systematic shift implies the residual direction does not sit symmetrically
%       relative to the model's neutral baseline.
%
%   (3) Negative α_R: Summarise whether coherent counter-emotional rewrites emerge,
%       and what this implies about the bipolarity of $\hat{\mathbf{r}}_e$.
%
%   (4) Updated practical conclusion (if the optimal α_R differs from 5):
%       "The practical operating point is therefore $\alpha_G = 0$,
%        $\alpha_R = \langle\text{opt}\rangle$."
