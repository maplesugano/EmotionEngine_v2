...
% -----------------------------------------------------------------
\section{Sign-Aligned-Mean Meta-Axes: \texorpdfstring{$5\times5$}{5x5} Pairwise Cosine Similarity Matrix}
\label{app:cosine_matrix}

Table~\ref{tab:cosine_matrix} gives the pairwise cosine similarities between the five sign-aligned-mean meta-axes $\mathbf{m}_1$--$\mathbf{m}_5$ at layer~13. Values near zero confirm near-orthogonality; the largest off-diagonal magnitude is $|\!\cos(\mathbf{m}_3,\mathbf{m}_4)| = 0.384$.
...
\hline
\end{tabular}
\caption{Pairwise cosine similarities between sign-aligned-mean meta-axes $\mathbf{m}_1$--$\mathbf{m}_5$ at layer~13 (\texttt{Llama-3.1-8B-Instruct}).}
...
The cross-method heatmap (Figure~\ref{fig:pooled_mk_heatmap}) in Chapter~\ref{chap:development2_preverbal_vectors} shows the absolute cosines between pooled-PCA components and these sign-aligned-mean axes; this table provides the underlying pairwise values for reproducibility.
...
\begin{description}
  \item[Dataset split seed] DailyDialog utterances retain their original corpus train/validation/test partitions. EmpatheticDialogues utterances (all labelled \texttt{all} in that corpus) form a separate pool; any downstream sub-sampling of this pool uses \texttt{random\_state=42} (scikit-learn \texttt{train\_test\_split}).
...
\begin{description}
  ...
  \item[CUDA] PyTorch build: CUDA~12.1 (\texttt{cu121}), cuDNN~8.9. NVIDIA driver supports CUDA~13.2.
\end{description}
...
After filtering, $22{,}782$ unique source utterances remained. These yield $546{,}768$ affective samples ($8$ emotions $\times$ $3$ intensity levels $\times$ $22{,}782$ source utterances) and $22{,}782$ neutral-paraphrase samples. Table~\ref{tab:dataset_split} reports the split sizes at the source utterance level, preserving the original corpus train/validation/test partitions from DailyDialog; EmpatheticDialogues utterances (labelled \texttt{all} in that corpus) form a separate pool.
...
\hline
\textbf{Total} & \textbf{22{,}782} & \textbf{546{,}768} \\
\hline
\end{tabular}
\caption{Dataset split sizes (source utterance level). Affective samples refer only to the $8{\times}3$ emotion rewrites. Neutral paraphrases were generated separately for all $22{,}782$ source utterances and partitioned using the same source-level boundaries.}
...
\begin{description}
  \item[Activation extraction] $\approx 18$~hours for the full 546{,}768-sample
...
\bottomrule
\end{tabular}
\caption{Summary of the $8\times8$ inter-emotion cosine similarity matrix at layer~13 before and after the \emph{full} decomposition pipeline (exp13, steps 1--3: projecting out both $\mathbf{g}$ and $\hat{\mathbf{r}}_e$). These values differ from Figure~\ref{fig:caa_decomposition_cosine_before_after_L13}, which reports the intermediate state after projecting out $\mathbf{g}$ only (off-diagonal mean $0.794$, range $0.73$--$0.89$); the additional prototype-projection step reduces the mean further to $0.787$ and tightens the range to $[0.730, 0.847]$.}
